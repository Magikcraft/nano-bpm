...
> they (a) upgrade `createProcessInstance` and job workers to the `/falcon`
...
{ "...": "standard Camunda fields",
  "nano": { "engine": "nanobpmn", "version": "0.1.0", "falconPath": "/falcon" } }
```
...
- `nano` **absent** ⇒ stock Camunda ⇒ pass-through (no WS, no migration). `nano`
  **present** ⇒ upgrade. The decorator caches the result and the broker directory
  (host:port + `falconPath`) for migration/failover targets.
...
- **Redirect:** when a `ServerFrame::Redirect` arrives on the Falcon protocol (server-side
...
- **`createProcessInstance`** (`gen/CamundaClient.ts:4437`): today it builds an envelope,
  ...
  create over the Falcon protocol (a `ClientFrame` create, correlated by `corr`, awaiting a
...
- **Job workers** — the high-leverage seam. `JobWorker._poll` depends on exactly **one**
  ...
  If the nano-aware client's `activateJobs` is fed by Falcon `Job` pushes (and
  ...
  **entire** worker loop — including `ThreadedJobWorker` and the `clientProxy`
  MessagePort path — upgrades to the Falcon protocol with **zero changes to worker code**.
...
## 4. The shared piece: a Falcon client module

The two seams are trivial wiring; the substance is one robust WS module
(`runtime/falcon.ts` in JS, mirrored per language). It owns:

- **Lifecycle:** connect to `${baseUrl-host}${falconPath}`, `Welcome`
...
- **Submission flow control:** honor `SubmissionCredits`; this supersedes the REST 429
  loop of `BackpressureManager` (`runtime/backpressure.ts`) when on the Falcon protocol
...
- **Validation, tenant defaults, telemetry, retry classification** all live *outside* the
  transport swap, so they apply identically on REST and Falcon paths.

## 6. Rollout (reference-first, then port)

- **S1 — JS detection + Falcon upgrade (testable now):** topology probe, the
  `falcon.ts` module, `createProcessInstance` + worker upgrade. Testable against a
  running nano gateway immediately — the server already speaks the Falcon protocol. No
...
Dependency note: S1/S2 depend only on shipped server behavior (Falcon +
...
- **No generator fork, no spec change for the client API.** The upgrade rides the existing
  Falcon WS (like `Pressure`/`SubmissionCredits`), not the OpenAPI surface.
...
- **Threaded worker + WS:** the `ThreadedJobWorker` runs handlers in worker_threads and
  proxies client calls over a MessagePort (`runtime/clientProxy.ts`). The Falcon
...
- **Long-poll semantics:** `activateJobs` long-poll `requestTimeout` has no direct WS
  analog; on the Falcon protocol, job delivery is push + credits, so the worker's
