...
A paradigm claim earns its keep only if it survives measurement. This section
reports what the prototype actually does, and — just as importantly — where its
wall is and *why*. A paper that names its own ceiling is more trustworthy than one
that does not; understanding our limit precisely is what turns it from an
embarrassment into the first line of the next chapter (§14). All figures below are
from real runs, logged in `PERFORMANCE.md` with the environment and topology so
they stay comparable; unmeasured figures elsewhere in the paper carry the
`[MEASURE]` mark, and none remain in this section.

### 12.1 Throughput, and the honest ceiling

The workload is the simplest thing that still exercises the whole loop: a
single-service-task process (`test-job-process.bpmn`), one job per instance, so
completed jobs/s *is* completed process-instances/s. Load is generated by coupled
producers and workers (`loadgen`, the Rust generator from `ts-performance-matrix`)
...
Aggregate throughput and per-request latency answer different questions, and Nano's
durability spectrum (§9.3) makes the distinction visible. On the synchronous-quorum
path a single low-concurrency worker sees a **hard latency floor of ~10 ms/job
(~100 jobs/s)** — one round-trip quorum commit, unpipelined. That same path
delivers **~33 000 jobs/s at 64 concurrent workers** (ADR 0003), because group
commit amortizes the quorum across many in-flight jobs. The floor is therefore a
property of *synchronous quorum on an unpipelined request*, not of Raft or of the
engine: it is the physics the operator is buying when they choose quorum-durable,
and §9.3's point is that they can choose otherwise (leader-durable, async) when the
floor, not the aggregate, is what constrains them. Reporting both numbers — and
naming which knob turns one into the other — is the evaluation being honest about a
trade rather than quoting only its best face.

### 12.3 Footprint under load and at rest

§8 argued footprint is a designed invariant; here is the evidence. The ADR 0012
soak — the canonical demonstration — drives a long unbounded-growth workload that
takes stock Zeebe-style terminal-state retention to **4.35 GB**, holds Nano at
**131 MB** steady, and returns it to **~0** once instances drain, because terminal
state is decoupled from compaction rather than pinned by the exporter cursor.
At the other end of the scale, a fresh release binary (empty, `console` feature)
idles at **~13 MB jemalloc resident / ~10 MB `phys_footprint`** — measured, not
estimated, on macOS via `footprint`, since `ps -o rss` under-reports there. The
two figures bound the claim from both ends: the engine is small when empty and
*stays bounded* when hammered, which is the property that makes the co-located
local-LLM future of §2.3 and Part II physically possible.

### 12.4 Compatibility as a generated, not asserted, property

Nano's central invariant (§4) is that it is a drop-in Camunda 8 replacement, and
the strongest evidence for that is architectural rather than anecdotal: the API
surface is **generated from the Camunda 8 OpenAPI/protocol specification**, not
hand-written to resemble it. Conformance is therefore something the build can be
held to, against the same C8 SDK conformance suites and the generated
orchestration-cluster-api SDKs that exercise a real cluster. This is the honest
scope line for the prototype: the surface is generated and the semantics that this
paper leans on (completion by key alone, §9.2; the create/activate/complete loop,
§6) are exercised by those clients, but full C8 conformance across every endpoint
is a suite to be run and reported, not a box already ticked. We claim
drop-in-*compatible by construction*, and treat the exhaustive conformance run as
part of the evaluation still owed.

### 12.5 Reproducibility

Every throughput figure above is a ~40–60 s steady-state window on the topology
named with it; the memory figures are a soak and a cold-start measurement with the
exact binary and tool stated. The load generator, the profiling switches
(`NANOBPMN_EXPORTER_PROFILE`, `NANOBPMN_READ_SYNC`), and the raw run log live in
the repository so the numbers — including the ceiling — can be reproduced and
attacked rather than taken on faith. An architecture that argues for legibility
(§11) should hold its own benchmarks to the same standard.
