...
### 13.3 The post‑LLM world: a confluence of paradigm‑shifting factors

Zeebe was architected before large language models were a design assumption; Nano
is architected after — and that difference is not one change but several arriving
at once. LLMs did not merely make Nano easier to write; they shifted the ground
under process automation itself, and Nano sits at the point where a number of
independent shifts converge.
...
developer's own workstation, and they are hungry — a capable local model claims
much of the machine's RAM. A 4.35 GB‑resident, JVM‑bound engine sitting *beside* a
local LLM is no longer merely heavy; it is untenable. Footprint (§8) therefore
stops being a nicety and becomes a **precondition** for the post‑LLM developer
workstation — the concrete pressure behind the "usable today" local‑development
claim of §1.

**Operability could finally be internalized.** When an engine cannot reason about
itself, the only place to put the reasoning is a human operator armed with knobs,
runbooks, and learned rules (§2) — the correct, and only, design under the prior
constraints. Once self‑optimization can absorb most of that surface (the
compressor/limiter decision, §5; the closed create→adapt→drain loops, §6; the
meter that replaces the runbook, §11), the operator's tuning burden moves inside
the engine. And **legibility becomes a first‑class design goal**: an engine small
and transparent enough for an LLM to reason about its state is a different
artifact from one that assumes only human operators will ever read its metrics.

**Using LLMs in concert with the engine pushed it further.** Pairing an LLM with a
process engine — to investigate incidents, propose model changes, and reason over
live behaviour (Part II, ProcessOS) — is a new mode of use that Zeebe was never
shaped for. It demands exactly the legibility and self‑description the point above
makes possible.
...
realities at once (§8), legible enough for an automated investigator to form and
test hypotheses over them. This need feeds straight back into the footprint
imperative: a 4.35 GB‑resident engine is the wrong size for a swarm of what‑ifs.

None of these factors alone would have produced Nano. Their **confluence** —
a decade of hard‑won engine knowledge, an order‑of‑magnitude drop in the cost of
building, memory pressure arriving at the very edge where the anomalies were
already felt (§13.1), and a new scientific use that demands a small, legible,
self‑optimizing engine — is the paradigm shift. Nano is what that confluence
builds.
