...
// Vendored fix (upstream PR GnomedDev/proc-macro-error-2#14): make the `proc_macro`
// extern crate publicly re-exportable so the `pub use proc_macro;` in the hidden
// `__export` module below stops tripping the E0365 future-incompatibility lint
// (private extern crate re-export, rust-lang/rust#127909). See the top-level repo
// note in server/Cargo.toml [patch.crates-io].
#[doc(hidden)]
