...
transitively (validator -> validator_derive -> proc-macro-error2). Nano bans
warnings project-wide, so we carry the one-line fix here and `[patch.crates-io]`
it from server/Cargo.toml until an upstream release ships it. Drop this vendored
copy and the patch entry once proc-macro-error2 > 2.0.1 is published with the fix.

Only src/lib.rs differs from the crates.io release.
-->
