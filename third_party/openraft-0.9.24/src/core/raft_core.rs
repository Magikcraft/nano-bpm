    ...
    SM: RaftStateMachine<C>,
{
    ...
    pub(crate) async fn apply_to_state_machine(
        ...
        // nanobpmn patch: guard `since >= end`, not just `since == end`.
        //
        // During a node's rejoin/snapshot-send race the leader can momentarily
        // compute a degenerate apply window where `since > end` (committed
        // briefly < last_applied). Upstream 0.9.24 only short-circuits the
        // `since == end` case, then calls `get_log_entries(since..end)` on the
        // inverted range. `check_range_matches_entries` treats it as empty and
        // returns Ok, so the empty `entries` slice reaches the unconditional
        // `entries[entries.len() - 1]` below and panics with a usize underflow,
        // killing this partition's RaftCore. Treating `since >= end` as a no-op
        // makes the degenerate window harmless (the next tick applies normally
        // once `committed` catches up).
        if since >= end {
            return Ok(());
        }
