...
# the model reproduces. Once the defect is fixed, TLC stops reporting that
...
for m in "${models[@]}"; do
  ...
  if [[ $code -eq 0 ]] && grep -q "Model checking completed. No error has been found." "$log"; then
    got="pass"
  elif v="$(grep -oE 'Invariant [A-Za-z0-9_]+ is violated' "$log" | head -1)" && [[ -n "$v" ]]; then
    got="violates:$(awk '{print $2}' <<<"$v")"
