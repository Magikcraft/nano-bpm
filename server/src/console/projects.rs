...
import type { WorkerInputs, WorkerOutputs, WorkerVars } from "./workers.d.ts";
...
"#;
/// Seed `.nanobpm/workers.d.ts` — the worker-IO type map (ADR 0033 §3). Starts
...
pub fn ensure_project_sdk(name: &str) -> std::io::Result<()> {
    ...
    // run like the rest of the SDK. Its IO map (`workers.d.ts`) is reified by the
    ...
    let workers_dts = nano.join("workers.d.ts");
...
pub fn create_project(
    ...
    w(dir.join(".nanobpm").join("workers.d.ts"), WORKERS_DTS_STUB)?;
    w(
...
mod tests {
    ...
    fn urban_template_scaffolds_app_manifest_and_dirs() {
        ...
        // `@nanobpm/worker` resolves to it (its IO map `workers.d.ts` is seeded).
        assert!(
            dir.join(".nanobpm/workers.ts").is_file(),
            ".nanobpm/workers.ts must be materialised"
        );
        assert!(
            dir.join(".nanobpm/workers.d.ts").is_file(),
            ".nanobpm/workers.d.ts must be seeded"
        );
        assert_eq!(
    ...
    async fn domaintypes_op_emits_typed_worker_bindings() {
        ...
        assert_eq!(dt["workerBindingsPath"], ".nanobpm/workers.d.ts");
        ...
        assert!(wb.contains("import type { DomainTypes } from \"./domain.d.ts\";"));

        let wb_disk = std::fs::read_to_string(dir.join(".nanobpm/workers.d.ts")).unwrap();
