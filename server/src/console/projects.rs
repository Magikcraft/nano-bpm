...
/// Seed `.nanobpm/domain.ts` — the typed data-object accessor (ADR 0029 §6). It
...
import type { WorkerInputs, WorkerOutputs, WorkerVars } from "./worker-io.d.ts";
...
"#;
/// Seed `.nanobpm/worker-io.d.ts` — the worker-IO type map (ADR 0033 §3). Starts
...
const PROJECT_DENO_JSON: &str = r#"{
  "imports": {
    ...
    "@nanobpm/domain": "./.nanobpm/domain.ts",
    "@lib/": "./lib/"
  },
  "tasks": {
...
const URBAN_DENO_JSON: &str = r#"{
  "imports": { "@nanobpm/nano-sdk": "npm:@nanobpm/nano-sdk@^1", "@nanobpm/worker": "./.nanobpm/workers.ts", "@nanobpm/llm": "./.nanobpm/llm-worker.ts", "@nanobpm/data": "./.nanobpm/data-sdk.ts", "@nanobpm/domain": "./.nanobpm/domain.ts", "@lib/": "./lib/" },
  "tasks": {
    "start": "deno run --allow-net --allow-read --allow-write --allow-env main.ts"
  }
}
...
pub fn ensure_project_sdk(name: &str) -> std::io::Result<()> {
    ...
    if !tsconfig.exists() {
        std::fs::write(&tsconfig, PROJECT_TSCONFIG_JSON)?;
    }
...
pub fn create_project(
    ...
    w(dir.join("tsconfig.json"), PROJECT_TSCONFIG_JSON)?;
    ...
    w(dir.join(".nanobpm").join("domain.ts"), DOMAIN_TS_STUB)?;
    w(dir.join(".nanobpm").join("workers.ts"), WORKERS_TS)?;
    w(
        dir.join(".nanobpm").join("worker-io.d.ts"),
        WORKERS_DTS_STUB,
    )?;
    w(
...
mod tests {
    ...
    fn urban_template_scaffolds_app_manifest_and_dirs() {
        ...
        assert!(
            dir.join(".nanobpm/domain.ts").is_file(),
            ".nanobpm/domain.ts must be seeded"
        );
        assert_eq!(
            deno_json["imports"]["@nanobpm/domain"], "./.nanobpm/domain.ts",
        ...
        assert!(
            dir.join(".nanobpm/workers.ts").is_file(),
            ".nanobpm/workers.ts must be materialised"
        );
        assert!(
            dir.join(".nanobpm/worker-io.d.ts").is_file(),
            ".nanobpm/worker-io.d.ts must be seeded"
        );
        assert_eq!(
            deno_json["imports"]["@nanobpm/worker"], "./.nanobpm/workers.ts",
        ...
        assert_eq!(
            tsconfig["compilerOptions"]["paths"]["@nanobpm/worker"][0], "./.nanobpm/workers.ts",
        ...
        assert!(
            !dir.join(".nanobpm/workers.d.ts").exists(),
    ...
    async fn export_bundles_regenerated_domain_types() {
        ...
        assert!(blob.contains("dtexport/.nanobpm/domain-rows.d.ts"));
    ...
    /// `.nanobpm/domain-rows.d.ts`: one interface per table + a `DomainTables` map,
    ...
    async fn domaintypes_op_reifies_the_schema_to_a_dts() {
        ...
        assert_eq!(dt["path"], ".nanobpm/domain-rows.d.ts");
        ...
        let on_disk = std::fs::read_to_string(dir.join(".nanobpm/domain-rows.d.ts")).unwrap();
        ...
        assert_eq!(dt["bindingsPath"], ".nanobpm/domain.ts");
        ...
        let bindings_disk = std::fs::read_to_string(dir.join(".nanobpm/domain.ts")).unwrap();
        assert_eq!(bindings_disk, bindings);

        // write:false returns the text without touching disk.
        std::fs::remove_file(dir.join(".nanobpm/domain-rows.d.ts")).unwrap();
        std::fs::remove_file(dir.join(".nanobpm/domain.ts")).unwrap();
        let dry = run_data_op(
            name,
        ...
        assert!(!dir.join(".nanobpm/domain-rows.d.ts").exists());
        ...
        let after_dry = std::fs::read_to_string(dir.join(".nanobpm/domain.ts")).unwrap();
    ...
    async fn domaintypes_op_emits_typed_worker_bindings() {
        ...
        assert_eq!(dt["workerBindingsPath"], ".nanobpm/worker-io.d.ts");
        ...
        // The type-less worker is not in either map.
        assert!(!wb.contains("\"noop\""));
        ...
        let wb_disk = std::fs::read_to_string(dir.join(".nanobpm/worker-io.d.ts")).unwrap();
        ...
        let wt = std::fs::read_to_string(dir.join(".nanobpm/workers.ts")).unwrap();
        assert_eq!(wt, WORKERS_TS);
        assert!(wt.contains("export function defineWorker<K extends string>("));
    }
