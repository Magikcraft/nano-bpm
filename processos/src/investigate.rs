...
// ADR-0002 slice 8: the optional `annotations` sidecar that used to
// piggyback on `edit_model` / `simulate` / `compare_variants` tool
// arguments has been removed from the LLM-facing tool schemas. The
// semantic-annotation channel is now owned end-to-end by the human
// Semantics Workbench (write path) plus the `read_annotations` tool
// (read path). The frontend still tolerates the field on legacy
// transcripts, but the LLM should never re-author it inline while
// proposing a model.
        ...
        if self.model.is_some() {
            ...
            specs.push(ToolSpec {
                ...
                description: "ALTERNATE REALITY ENGINE — score a MULTIVERSE of candidate models \
                    ...
                    mockWorkers?}), includeBaseline (bool), mockWorkers (shared map), \
                    limit (replay only the first N instances)."
                    .into(),
                parameters: json!({
                    "type": "object",
                    "properties": {
