...
(function () {
  ...
  var CSS = [
    '#cog{position:fixed;left:14px;bottom:14px;width:38px;height:38px;border-radius:50%;font-size:18px;line-height:1;display:flex;align-items:center;justify-content:center;background:var(--po-hover);border:1px solid var(--po-edge-strong);color:var(--po-text-muted);cursor:pointer;z-index:50;padding:0}',
    '#cog:hover{color:var(--po-text);border-color:var(--po-accent-strong)}',
    '#settings-overlay{position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:60;display:flex}',
    '#settings-overlay[hidden]{display:none}',
    '#settings-panel{margin:0 auto 0 0;width:380px;max-width:94vw;height:100%;background:var(--po-panel);border-right:1px solid var(--po-edge);box-shadow:0 0 40px var(--po-shadow);display:flex;flex-direction:column;color:var(--po-text);font:14px/1.5 system-ui,sans-serif}',
    '#settings-panel .ph{display:flex;align-items:center;justify-content:space-between;padding:14px 16px;border-bottom:1px solid var(--po-edge)}',
    '#settings-panel .ph button{background:none;border:none;color:var(--po-text-muted);font-size:16px;cursor:pointer}',
    '.settings-body{padding:16px;display:flex;flex-direction:column;gap:12px;overflow:auto}',
    '.settings-body h4{margin:6px 0 0;font-size:11px;text-transform:uppercase;letter-spacing:.06em;color:var(--po-text-faint)}',
    '.settings-body label{display:flex;flex-direction:column;gap:4px;font-size:12px;color:var(--po-text-muted)}',
    '.settings-body input,.settings-body select{background:var(--po-inset);border:1px solid var(--po-edge-strong);border-radius:6px;color:var(--po-text);padding:6px 8px;font:inherit}',
    '.settings-body .row{display:flex;gap:8px}',
    '.settings-body .row label,.settings-body .row>*{flex:1}',
    '.settings-body button{background:var(--po-hover);color:var(--po-text);border:1px solid var(--po-edge-strong);border-radius:6px;padding:6px 10px;cursor:pointer;font:inherit}',
    '.settings-body button.primary{background:var(--po-accent-wash);border-color:var(--po-accent-strong);color:var(--po-accent-soft)}',
    '.settings-body button.ghost{background:none}',
    '.settings-body .bar{display:flex;gap:8px;align-items:center;flex-wrap:wrap}',
    '.settings-body .sep{height:1px;background:var(--po-edge);margin:6px 0}',
    '#s-status{color:var(--po-text-muted);font-size:12px;min-height:16px}',
    '#s-path{color:var(--po-text-faint);font-size:11px;word-break:break-all}',
    '.s-active-tag{font-size:10px;color:var(--po-ok-soft);border:1px solid var(--po-ok-edge);border-radius:999px;padding:0 7px}',
    '.settings-body .hint{font-size:11px;color:var(--po-text-faint)}',
    '.settings-body .hint a{color:var(--po-accent-strong)}',
    '.settings-body label.ck{flex-direction:row;align-items:center;gap:7px;color:var(--po-text);cursor:pointer}',
    '.settings-body label.ck input{flex:0 0 auto;width:auto}',
    '.settings-body .s-types{display:flex;gap:10px;margin:2px 0 2px}',
    '.settings-body label.rd{flex:1;flex-direction:row;align-items:center;gap:7px;color:var(--po-text);cursor:pointer;background:var(--po-inset);border:1px solid var(--po-edge-strong);border-radius:6px;padding:7px 9px;font-size:13px}',
    '.settings-body label.rd input{flex:0 0 auto;width:auto}',
    '.settings-body label.rd.sel{border-color:var(--po-accent-strong);background:var(--po-accent-wash)}',
    ...
    '#s-llama-status{font-size:12px;color:var(--po-text-muted);min-height:16px}',
    '.s-dot{display:inline-block;width:8px;height:8px;border-radius:50%;background:var(--po-text-faint);margin-right:6px;vertical-align:middle}',
    '.s-dot.on{background:var(--po-ok)}',
    ...
    '#llama-logs-panel{margin:auto;width:780px;max-width:94vw;height:80vh;background:var(--po-panel);border:1px solid var(--po-edge);border-radius:8px;display:flex;flex-direction:column;color:var(--po-text);font:13px/1.5 system-ui,sans-serif}',
    '#llama-logs-panel .llh{display:flex;justify-content:space-between;align-items:center;padding:10px 14px;border-bottom:1px solid var(--po-edge)}',
    '#llama-logs-panel .llh strong{font-size:13px}',
    '#llama-logs-panel .llh button{background:none;border:none;color:var(--po-text-muted);font-size:16px;cursor:pointer}',
    '.ll-cmd{padding:8px 14px;font:12px/1.45 ui-monospace,Menlo,monospace;color:var(--po-accent-strong);border-bottom:1px solid var(--po-edge);word-break:break-all;background:var(--po-inset)}',
    '.ll-cmd .lbl{display:block;color:var(--po-text-faint);font-family:system-ui,sans-serif;font-size:10px;text-transform:uppercase;letter-spacing:.06em;margin-bottom:3px}',
    '#ll-body{margin:0;padding:10px 14px;overflow:auto;flex:1;font:12px/1.45 ui-monospace,Menlo,monospace;color:var(--po-text);white-space:pre-wrap;word-break:break-word}'
  ].join('\n');
