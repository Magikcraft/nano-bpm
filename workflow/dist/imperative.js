...
export function defineWorkflow(id, orchestrate) {
    ...
    if (typeof orchestrate !== "function")
        throw new Error("defineWorkflow needs an async orchestration function");
...
export function imperativeToBpmn(wf) {
 ...
 */
export async function replayOnce(wf, input, journal) {
    let ordinal = 0;
    const ctx = {
        input,
        async run(name, fn) {
            ...
            if (Object.prototype.hasOwnProperty.call(journal, key)) {
                return journal[key]; // replay: return recorded value, no side effect
            }
            const result = ((await fn()) ?? null); // frontier: the ONE real side effect this turn
            const carrier = new Error("nanobpm.workflow suspend");
