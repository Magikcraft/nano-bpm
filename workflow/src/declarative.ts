...
import type { DeclarativeFlow, DeclarativeStep, StepHandler } from "./types.js";
...
export interface FlowBuilder {
   ...
   */
  signal(name: string, opts: { correlationKey: string }): FlowBuilder;
}
...
export function declarativeToBpmn(flow: DeclarativeFlow): string {
  ...
  flow.steps.forEach((step, i) => {
    ...
    if (step.kind === "run" || step.kind === "task") {
      ...
      nodes.push(
        `    <bpmn:intermediateCatchEvent id="${escapeXml(step.name)}" name="${escapeXml(step.name)}">\n` +
          `      <bpmn:incoming>${incoming}</bpmn:incoming><bpmn:outgoing>${outgoing}</bpmn:outgoing>\n` +
          `      <bpmn:messageEventDefinition messageRef="${msgId}" />\n` +
          `    </bpmn:intermediateCatchEvent>`,
      );
      messages.push(
        `  <bpmn:message id="${msgId}" name="${escapeXml(messageName(flow.id, step.name))}">\n` +
