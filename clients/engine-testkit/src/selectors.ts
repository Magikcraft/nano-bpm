...
// so the two matchers cannot drift. It is fully deterministic: it reads the
 ...
 *  be selected). Exported so the sibling matchers — and unit tests — can read the
 ...
 *  {@link resolveInstanceKey} so its pure logic is unit-testable without an engine. */
...
/** Resolve `selectorOrKey` against `engine.snapshot()`, returning the concrete
 *  process-instance key. Used by the instance and user-task matchers. */
export function resolveInstanceKey(
  engine: EngineReadModel,
  selectorOrKey?: string | InstanceSelector,
): string {
  return resolveFromInstances(readInstances(engine.snapshot()), selectorOrKey);
}
