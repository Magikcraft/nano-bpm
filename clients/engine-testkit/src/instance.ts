...
//
// Every matcher reads synchronously from `engine.snapshot()` (the engine's parsed
...
import type { EngineReadModel } from "./port.ts";
import type { ProcessInstanceState } from "./state.ts";
import { wasmStateToProcessInstanceState } from "./state.ts";
...
function instanceRecord(engine: EngineReadModel, key: string): Record<string, unknown> {
 ...
 *  and tests assert against an isolated single-instance snapshot per case. */
...
export function assertThatInstance(
  ...
  const self: InstanceAssert = {
    ...
    hasCompletedElements: (first, ...rest) => {
      ...
      // Refuse in both cases (tests scope the happy path to an isolated snapshot).
    ...
    hasVariable: (name, value) => {
      const vars = variablesOf(instanceRecord(engine, key));
    ...
    hasIncident: (selector) => {
      const incidents = incidentsFor(engine.snapshot(), key);
