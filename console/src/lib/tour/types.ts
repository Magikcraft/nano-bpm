...
export interface HandoffStep extends StepBase {
   ...
   * mere step advance). Lets a first-party journey latch its outcome signal — e.g.
   * journey 1's "the base URL was taken" — from the real copy action rather than
   * guessing from step completion. Absent from pack-contributed journeys by
   * construction: those arrive as JSON data and cannot carry a function, so this
   * is never a pack execution vector.
   */
  onCopied?: () => void;
  /**
