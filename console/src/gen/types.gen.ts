...
/**
 * Console-integrated app-view descriptor (ADR 0057, issue #638), resolved from the optional `ui` block in the app manifest against the project's run env. The studio uses it to place a running app in the left rail and, for a UI app, to locate its embedded webview. An app with no resolvable UI port is headless: still listed, but shown as a control-only pane. Identity/label/icon are display hints only — apps scaffolded from the same template share an identical manifest — so callers key the nav entry on the project name, not on these fields.
 */
export type AppUi = {
    /**
     * Whether the app opts in to an embedded UI. false (or no resolvable port) means headless: still listed, but control-only.
     */
    enabled: boolean;
    /**
    ...
    port?: number | null;
    /**
     * Path the embedded webview should open (default "/").
     */
    path?: string | null;
    /**
     * Left-rail icon hint (bundled icon name). The console falls back to a default glyph when absent/invalid.
     */
    icon?: string | null;
    /**
     * Left-rail label hint. The console disambiguates on the project name.
     */
    label?: string | null;
};
...
export type ProjectDetail = {
    ...
    runState: RunState;
    /**
     * The console-integrated app-view descriptor (ADR 0057, issue #638). Always present; `enabled: false` or a null `port` marks a headless app (listed in the running-apps rail, but control-only).
     */
    appUi?: AppUi;
    /**
